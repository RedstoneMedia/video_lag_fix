...
def benchmark(imgs_path: str, iter_count = 100):
    server = ImageProcessServerConnect("image_cache", False, filter_type="Lanczos3", grayscale=True)
    img_paths = [os.path.join(imgs_path, name) for name in
                 os.listdir(imgs_path)]
    print(len(img_paths))
    # Prevent cold start
    start = time.time()
    imgs = server.ask_for_images(img_paths, width=400, height=255)
    ...
    for i in range(iter_count):
        print(i)
        start = time.time()
        imgs = server.ask_for_images(img_paths, width=320, height=180)
        assert len(imgs) == len(img_paths)
        took = time.time() - start
        total_time += took
    avg_time = total_time/iter_count
    print(f"Took: {avg_time:.4f}s/iter, {avg_time / len(img_paths) * 1000:.4f}ms/image")
